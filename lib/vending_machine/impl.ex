defmodule VendingMachine.Impl do
  ...
  def give_change(vending_machine, amount_owed) do
    cond do
      amount_owed >= 25 ->
        {coin, _rest} = VendingMachine.CoinStorage.remove_any(vending_machine.staging)
      ...
      amount_owed >= 10 ->
        coin = VendingMachine.CoinStorage.get_highest_non_quarter_coin(vending_machine.bank)
        vending_machine = transfer_coin(vending_machine, :bank, :coin_return, coin)
        give_change(vending_machine, amount_owed - get_value_of_coin(coin))

      amount_owed >= 5 ->
        transfer_coin(vending_machine, :bank, :coin_return, VendingMachine.Coin.createNickel())
  ...
  def remove_highest_non_quarter_coin(vending_machine) do
    if Enum.any?(vending_machine.bank, fn coin -> coin == VendingMachine.Coin.createDime() end) do
      {VendingMachine.Coin.createDime(),
       put_in(vending_machine.bank, vending_machine.bank -- [VendingMachine.Coin.createDime()])}
    else
      {VendingMachine.Coin.createNickel(),
       put_in(vending_machine.bank, vending_machine.bank -- [VendingMachine.Coin.createNickel()])}
    end
  end
  ...
  def transfer_coin(vending_machine, from, to, coin) do
    vending_machine =
      put_in(
        vending_machine,
        [Access.key(from)],
        VendingMachine.CoinStorage.remove_coin(get_in(vending_machine, [Access.key(from)]), coin)
      )
    ...
    if to == :coin_return do
      ...
      put_in(
        vending_machine,
        [Access.key(to)],
        VendingMachine.CoinStorage.add_coin(get_in(vending_machine, [Access.key(to)]), coin)
      )
    end
  end
